...
def data_manipulation():
    Acc=[ [ (0.0,j) for j in range(0,55)] for i in [1,2,3,4,5,6,7,8]]
    print Acc
    for j in range(46,47):
        # if(j==30 or j==32 or j==35 or j==39 or j==52):   
        #     continue
        sys.stderr.write('experiment no. '+str(j)+' \n')
        linear_accuracy = []
        logistic_accuracy = []
        svr_accuracy = []
        svm_accuracy = []
        ...
        for i in [1,2,3,4,5,6,7,8]: #to change after feature extraction done for all sets
            ...
            # print '/home/ankit/Automated_Essay_Grading/experiment/experiment_'+str(j)+'_features_'+str(j)+'.csv'
            with open('/home/ankit/Automated_Essay_Grading/increment_feature/incr_'+str(j)+'_features_'+str(i)+'.csv','r') as in_file:
            ...
            decision_tree_accuracy.append(decision_class_k_cross.execute(i,4))
            ...
            Acc[i-1][j]=(np.average(linear_accuracy),j)
            print 
    print Acc
    print "super"
    for i in [1,2,3,4,5,6,7,8]:
        print
        print sorted(Acc[i-1])
        print 
...
def predict(f,i,j):
    ...
    generate_feature(f,j)
    print "generate_feature_done"
    with open('dumped_dim_red_'+str(j)+'.pkl', 'rb') as fid:
        dim_red = cPickle.load(fid)

    with open('dumped_'+str(i)+'_'+str(j)+'.pkl', 'rb') as fid:
    ...
    val = int(np.round(stat_obj.predict(x_test)))
    print " experiment "+str(j)+" :"
    print " linear_regression :\t" +  str(np.average(linear_accuracy)) + '\t'+str(linear_accuracy)
    print " logistic_regression :\t" +str(np.average(logistic_accuracy)) + '\t'+ str(logistic_accuracy)
    print " support_vector_regression :\t" +str(np.average(svr_accuracy)) + '\t'+ str(svr_accuracy)
