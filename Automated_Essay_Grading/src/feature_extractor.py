...
words_acq_age = json.load(open('../data/aoa_values.json'))
    ...
    def stylized_word_scores(self):
        ...
        for word in words:
            ...
            if lower_word in words_acq_age and len(lower_word) > 3:
                ...
                vocab += 1
        # print "dsgfdfdhfd",len(self.uniq)
        # print "Dsggsdg"
...
def make_points():
    ...
    '''
    # currently performing tasks on essay set 1 only
    for index in xrange(1,9):
        with open('../data/training_set_' + str(index) + '.csv','rb') as f:
            all_words = []
            essays = []
            ...
            st=points[0].get_label()[:-1]
            st=st.split(',')
            # num_features=len(st)-3
            
            features_to_use=[4, 1, 5, 52, 11, 33, 19, 17, 30, 6, 0, 12, 39, 34, 16, 21, 44, 7, 46, 43, 3, 37, 42, 38, 35, 51, 13, 45, 20, 8, 10, 9, 2, 47, 50, 53, 26, 48, 24, 31, 28, 40, 18, 41, 23, 54, 14, 22, 25, 15, 49, 36, 32, 29, 27]
            for fr in range(len(features_to_use)):
                if(fr ==50):
                    break
                # use features from 0:fr+1 #increment_feature
                out_file = open('../increment_feature/incr_'+str(fr)+'_features_' + str(index) + '.csv','w')
                lc=''
                for indx in range(fr+1):
                    lc+=','+str(st[features_to_use[indx]+3]) 
                out_file.write('id,set,human_score'+lc+'\n')
            for p in points:
                ft=str(p)[:-1]
                ft=ft.split(',')
                features_to_use=[4, 1, 5, 52, 11, 33, 19, 17, 30, 6, 0, 12, 39, 34, 16, 21, 44, 7, 46, 43, 3, 37, 42, 38, 35, 51, 13, 45, 20, 8, 10, 9, 2, 47, 50, 53, 26, 48, 24, 31, 28, 40, 18, 41, 23, 54, 14, 22, 25, 15, 49, 36, 32, 29, 27]
                for fr in range(len(features_to_use)):
                    if(fr ==50):
                        break
                    # use features from 0:fr+1 #increment_feature
                    out_file = open('../increment_feature/incr_'+str(fr)+'_features_' + str(index) + '.csv','a')
                    lc=''
                    for indx in range(fr+1):
                        lc+=','+str(ft[features_to_use[indx]+3]) 
                    out_file.write(ft[0]+','+ft[1]+','+ft[2]+lc+'\n')
                    out_file.close()
